...
first_row = ['月份', '指数', '环比(%)', '指数', '环比(%)', '指数', '环比(%)', '指数', '环比(%)']
city = ['北京','上海','广州','深圳','天津','武汉','重庆','南京','杭州','成都']

with open('./XinFang.csv', mode='w', encoding='mbcs', newline='') as file_obj:
	...
	for k in range(len(city)):  
		...
		url = 'https://fdc.fang.com/index/XinFangIndex.aspx?action=city&city=' + cityuni
